# The estimation of a regression of ln(vio), ln(rob), ln(mur)
...
for j in y_columns_ls:
    y = pd.DataFrame(df, columns=[j]).to_numpy()   # 'vio', 'mur', 'rob'
    ...
    for i in x_columns_ls:
        ...
        write_into_file('guns_output.txt', 'ln(' + str(j) +
                        ')' + '   vs  ' + str(i) + '\n')
